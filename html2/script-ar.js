...
document.querySelectorAll('.nav-link').forEach(link => {
    link.addEventListener('click', (e) => {
        ...
        // For external links (like index.html), let the browser handle it normally
    });
});
...
const carouselData = [
    {
        ...
        description: 'صفّ السيارات مجانًا لـ 15 مرة شهريًا'
    },
    {
        image: 'assets/images/gym-2.jpg',
        title: 'اللياقة والصحة',
        description: 'خصم 50% على اشتراكات صالات الألعاب الرياضية واللياقة البدنية الشريكة'
    },
    {
        image: 'assets/images/cinema-2.jpg',
        title: 'عروض السينما',
        description: 'عرض اشترِ تذكرة واحصل على الثانية مجانًا في نوفو سينما (بحد أقصى 4 تذاكر شهريًا، تشمل فئتي المجلس و7 نجوم)'
    },
    {
        image: 'assets/images/club-2.jpg',
        title: 'امتيازات الأندية',
        description: 'خصم 50% على عضوية نادي "ذا نيد الدوحة" ونادي "سند" (حسب الدعوة)'
    }
];
...
// Privileges Slider Functionality
...
function privilegesGoToSlide(index) {
    privilegesCurrentSlide = index;
    privilegesShowSlide(index);
}

// Initialize privileges slider dots
document.addEventListener('DOMContentLoaded', () => {
    const dotsContainer = document.getElementById('privilegesSliderDots');
    if (dotsContainer && privilegesTotalSlides > 0) {
        for (let i = 0; i < privilegesTotalSlides; i++) {
            ...
            dot.onclick = () => privilegesGoToSlide(i);
            dotsContainer.appendChild(dot);
        }
    }
});

// Auto-advance privileges slider every 5 seconds
setInterval(() => {
    if (privilegesTotalSlides > 0) {
        privilegesNext();
    }
}, 5000);
...
function handleFormSubmit(event) {
    ...
    alert('شكراً لك على تقديم طلبك! سيتواصل معك فريق الخدمات المصرفية المميزة قريباً.');
